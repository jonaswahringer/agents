...
# The source reports show paths relative to home, and call out a broken link on
# its own line instead of hiding it in the table.
"$AGENTS" source > "$TEST_ROOT/source.txt"
assert_contains "$TEST_ROOT/source.txt" "~/.local/share/agents/source"
assert_contains "$TEST_ROOT/source.txt" "~/.local/bin/agents  →  ~/.local/share/agents/source/bin/agents"
assert_contains "$TEST_ROOT/source.txt" "~/.claude/CLAUDE.md  →  ~/.agents/AGENTS.md"
assert_contains "$TEST_ROOT/source.txt" "shared file, generated by agents"
"$AGENTS" skills source > "$TEST_ROOT/skills-source.txt"
assert_contains "$TEST_ROOT/skills-source.txt" "~/.local/share/agents/source/skills"
assert_contains "$TEST_ROOT/skills-source.txt" "→  jonasw/commit"
assert_not_contains "$TEST_ROOT/skills-source.txt" "!"
rm "$TEST_HOME/.codex/skills/commit"
"$AGENTS" skills source > "$TEST_ROOT/skills-source-broken.txt"
assert_contains "$TEST_ROOT/skills-source-broken.txt" "! ~/.codex/skills/commit is missing"
ln -s "$TEST_HOME/.agents/skills/commit" "$TEST_HOME/.codex/skills/commit"
"$AGENTS" doctor --verbose > "$TEST_ROOT/verbose-doctor.txt" || fail "verbose doctor should pass on a fresh install"
assert_contains "$TEST_ROOT/verbose-doctor.txt" "Global instructions"
assert_contains "$TEST_ROOT/verbose-doctor.txt" "Skill links"
if "$AGENTS" doctor --bogus >/dev/null 2>&1; then
  fail "doctor should reject an unknown option"
fi
